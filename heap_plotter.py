...
def create_sizing_parameters_summary(sizing_entries):
    ...
    uncommit_entries = [e for e in sizing_entries if
                        e.sizing_type == 'time_based_uncommit' and e.uncommit_mb and e.uncommit_mb > 0]
    ...
    print(
        f"DEBUG: Sizing summary - entries={len(uncommit_entries)}, timestamps={len(common_timestamps)}, memory range: {min(memory_reclaimed) if memory_reclaimed else 0}-{max(memory_reclaimed) if memory_reclaimed else 0}")
...
def create_heap_evaluation_timeline(sizing_entries):
    ...
    uncommit_entries = [e for e in sizing_entries if e.sizing_type in ['time_based_uncommit',
                                                                       'heap_shrink_completed'] and e.uncommit_mb and e.uncommit_mb > 0]
    shrink_entries = [e for e in sizing_entries if e.sizing_type in ['heap_shrink_completed',
                                                                     'heap_shrink_details'] and e.uncommit_mb and e.uncommit_mb > 0]
    ...
    fig = make_subplots(
        rows=3, cols=1,
        shared_xaxes=True,
        subplot_titles=('Heap Sizing Evaluation Decisions', 'Evaluation Outcomes & Actions',
                        'Heap Size After Shrink Operations'),
...
def create_region_transitions_plot(sizing_entries):
    ...
    uncommit_entries = [e for e in sizing_entries if
                        e.sizing_type == 'time_based_uncommit' and e.uncommit_mb and e.uncommit_mb > 0]
    eval_entries = [e for e in sizing_entries if
                    e.sizing_type in ['time_based_evaluation_shrink', 'time_based_evaluation_no_uncommit']]
    ...
    remained_inactive = [max(0, inactive - uncommitted) for inactive, uncommitted in
                         zip(inactive_regions, uncommitted_regions)]
...
def parse_gc_log(log_content):
    ...
    jdk_version_pattern = re.compile(
        r"\[\d+\.\d+s\]\[info\]\[gc,init\] Version: (\d+\.\d+\.\d+\+\d+-LTS(?:-\d+)?) \(release\)")
    g1_patterns = [
...
def generate_plot(data_df, scaling_data_df, selected_value):
    ...
    # Handle the 'summary' plot option
        ...
        table_trace = go.Table(
            header=dict(
                values=['Pause Name', 'Count', 'Min Time (ms)', 'Max Time (ms)', 'Total Time (ms)', 'Overhead (%)']),
            cells=dict(values=[
                pause_summary.index,
                pause_summary['count'],
